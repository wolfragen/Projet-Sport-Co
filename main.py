...
if(__name__ == "__main__"):
    ...
    if MODE == 0:
        ...
        Settings.ENTRY_NEURONS = 10  # solo: no other players
    ...
    # ======================================================================
        ...
        players_number = (1, 1)
        Settings.PLAYERS_NUMBER = players_number
        Settings.ENTRY_NEURONS = 10 + (sum(players_number) - 1) * 2  # = 12
    ...
    # ======================================================================
        ...
        Settings.ENTRY_NEURONS = 10 + (n_players - 1) * 3  # = 19 (dx, dy, team_flag per other player)

        dimensions_actor = (Settings.ENTRY_NEURONS, 128, 64, 64, 32, 4)
        dimensions_critic = (4 + n_players * 5, 128, 128, 64, 64, 1)  # global state: ball(4) + 4*player(5)

        reward_coeff_dict = {
            "static_lead_reward": 0.003,
            "static_draw_reward": -0.001,
            "delta_ball_player_coeff": 0.002,  # faible: bootstrap l'approche sans ball-chasing
            "delta_ball_goal_coeff": 0.02,
            "can_shoot_coeff": 0.2,
            "goal_coeff": 1,
            "wrong_goal_coeff": -1,
        ...
        # team: n_left players per env, so effective virtual envs = N_WORKERS * n_left
        agent = PPOAgent(
            dimensions=(dimensions_actor, dimensions_critic),
            scoring_function=scoring_function, reward_coeff_dict=reward_coeff_dict,
        ...
        """ team_training(
            ...
            max_steps_per_game=2048, eval_interval=1024,
            rolling_size=4096,
            team_reward_ratio=0.9,  # 0.1*individual + 0.9*team (anti ball-chasing)
        ) """
        
        random_agent = RandomAgent(action_dim=4)
        agent.load(save_folder + "team/actor_43.pt")

        # --- Testing ---
        # agents = [agent, agent, random_agent, random_agent]
        # runTests(players_number=players_number, agents=agents, max_steps=2048, nb_tests=1000,
        #          scoring_function=scoring_function, reward_coeff_dict=reward_coeff_dict)
