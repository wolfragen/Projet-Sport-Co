...
<<<<<<< HEAD
    players_number: tuple[int, int] = (1,0)
    
    dimensions: tuple[int, int, int, int] = (Settings.ENTRY_NEURONS, 128, 128, 5)
    sync_rate: int = 1000
    batch_size: int = 128
    lr: float = 5e-5
    gamma: float = 0.99
    ...
    wait_rate: int = 0
    exploration_rate: float = 0.5 - wait_rate # à x%, on atteint le min d'epsilon, en incluant le temps "stagnant"
    num_wait: int = round(num_episodes*wait_rate) # number of episodes to wait until epsilon decay
    
    starting_max_steps: int = 750
    ending_max_steps: int = 750
    ...
    epsilon_decay: float = (epsilon - epsilon_min) / ((num_episodes-num_wait)*exploration_rate)
